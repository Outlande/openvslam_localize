...
system::system(const std::shared_ptr<config> &cfg,
  ...
  message_stream << R"(  ___               __   _____ _      _   __  __ )"
                 << std::endl;
  message_stream << R"( / _ \ _ __  ___ _ _\ \ / / __| |    /_\ |  \/  |)"
                 << std::endl;
  message_stream << R"(| (_) | '_ \/ -_) ' \\ V /\__ \ |__ / _ \| |\/| |)"
                 << std::endl;
  message_stream << R"( \___/| .__/\___|_||_|\_/ |___/____/_/ \_\_|  |_|)"
                 << std::endl;
  message_stream << R"(      |_|                                        )"
                 << std::endl;
  message_stream << std::endl;
  message_stream << "Copyright (C) 2019," << std::endl;
  message_stream << "National Institute of Advanced Industrial Science and "
                    "Technology (AIST)"
                 << std::endl;
  ...
  frame_publisher_ = std::shared_ptr<publish::frame_publisher>(
      new publish::frame_publisher(cfg_, map_db_));
  map_publisher_ = std::shared_ptr<publish::map_publisher>(
      new publish::map_publisher(cfg_, map_db_));

  // tracking module
  tracker_ = new tracking_module(cfg_, this, map_db_, bow_vocab_, bow_db_);
  // mapping module
  mapper_ = new mapping_module(map_db_, camera_->setup_type_ ==
                                            camera::setup_type_t::Monocular);
  // global optimization module
  global_optimizer_ = new global_optimization_module(
      map_db_, bow_db_, bow_vocab_, cfg_->yaml_node_,
...
void system::startup(const bool need_initialize) {
  ...
  mapping_thread_ = std::unique_ptr<std::thread>(
      new std::thread(&openvslam::mapping_module::run, mapper_));
  global_optimization_thread_ = std::unique_ptr<std::thread>(new std::thread(
      &openvslam::global_optimization_module::run, global_optimizer_));
}
...
void system::shutdown() {
  ...
  while (!mapper_->is_terminated() || !global_optimizer_->is_terminated() ||
...
void system::save_keyframe_trajectory(const std::string &path,
                                      const std::string &format) const {
  pause_other_threads();
  io::trajectory_io trajectory_io(map_db_);
...
const std::shared_ptr<publish::map_publisher>
system::get_map_publisher() const {
  return map_publisher_;
}

const std::shared_ptr<publish::frame_publisher>
system::get_frame_publisher() const {
...
void system::enable_mapping_module() {
  ...
  if (!system_is_running_) {
    spdlog::critical(
        "please call system::enable_mapping_module() after system::startup()");
  }
...
void system::disable_mapping_module() {
  ...
  if (!system_is_running_) {
    spdlog::critical(
        "please call system::disable_mapping_module() after system::startup()");
  }
...
bool system::mapping_module_is_enabled() const { return !mapper_->is_paused(); }
...
void system::abort_loop_BA() { global_optimizer_->abort_loop_BA(); }

Mat44_t system::feed_monocular_frame(const cv::Mat &img, const double timestamp,
                                     const cv::Mat &mask) {
  assert(camera_->setup_type_ == camera::setup_type_t::Monocular);
  check_reset_request();
  const Mat44_t cam_pose_cw =
      tracker_->track_monocular_image(img, timestamp, mask);
...
Mat44_t system::feed_stereo_frame(const cv::Mat &left_img,
  ...
  const Mat44_t cam_pose_cw =
      tracker_->track_stereo_image(left_img, right_img, timestamp, mask);
...
Mat44_t system::feed_RGBD_frame(const cv::Mat &rgb_img, const cv::Mat &depthmap,
                                const double timestamp, const cv::Mat &mask) {
  ...
  const Mat44_t cam_pose_cw =
      tracker_->track_RGBD_image(rgb_img, depthmap, timestamp, mask);
...
void system::pause_tracker() { tracker_->request_pause(); }

bool system::tracker_is_paused() const { return tracker_->is_paused(); }

void system::resume_tracker() { tracker_->resume(); }
...
void system::pause_other_threads() const {
  ...
  if (global_optimizer_ && !global_optimizer_->is_terminated()) {
    ...
    while (!global_optimizer_->is_paused() &&
